    ...
    return {
      ...
      rules: {
        oldPassword: [
          {
            required: true,
            message: this.$t("profile.emptyOldPassword"),
            trigger: "blur",
          },
        ],
        newPassword: [
          {
            required: true,
            message: this.$t("profile.emptyNewPassword"),
            trigger: "blur",
          },
        ...
        newPassword2: [
          {
            required: true,
            message: this.$t("profile.emptyConfirmPassword"),
            trigger: "blur",
          },
  ...
  methods: {
    ...
    change() {
      this.$refs["dataForm"].validate((valid) => {
        if (!valid) return;
        this.$api("admin.setpwd", this.dataForm)
          .then((response) => {
