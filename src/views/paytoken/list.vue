<template>
  <div class="app-container">
    ...
    <el-table
      ...
      :data="list"
      :element-loading-text="$t('payload.loadingText')"
      fit
    ...
    >
      ...
      <el-table-column align="center" :label="$t('paytoken.default')">
        <template slot-scope="scope">
          <span v-if="!scope.row.isDefault">{{ $t("paytoken.no") }}</span>
          <span v-else>{{ $t("paytoken.yes") }}</span>
      ...
      >
        ...
        >
          <!-- <el-checkbox
            ...
            :label="$t('paytoken.default')"
          ></el-checkbox> -->
          <el-select v-model="dataForm.isDefault">
            <el-option
              v-for="type in isDefault"
              :key="type.value"
              :label="type.name"
              :value="type.value"
            >
    ...
    return {
      ...
      rules: {
        name: [
          {
            required: true,
            message: this.$t("paytoken.emptyName"),
            trigger: "blur",
          },
        ],
        symbol: [
          {
            required: true,
            message: this.$t("paytoken.emptySymbol"),
            trigger: "blur",
          },
        ],
        decimals: [
          {
            required: true,
            message: this.$t("paytoken.emptyDecimals"),
            trigger: "blur",
          },
        ],
        address: [
